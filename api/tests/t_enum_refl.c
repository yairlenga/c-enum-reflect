#include <stdio.h>

#include "enum_desc.h"

enum e1 { E1=1, E3=3, E100=100} ;

enum s2 { VV1 = 10, VV2=20, VV3=-30, VV4=12345 } ;

static void show_desc(enum_desc_t ed)
{
    int item_count = enum_desc_item_count(ed) ;
    printf("Enum '%s' %d items\n", enum_desc_name(ed), item_count) ;
    for (int i=0 ; i<item_count ; i++ ) {
        printf("#%d: %d (%s)\n", i, enum_desc_value_at(ed, i), enum_desc_label_at(ed, i)) ;
    }
}

static void test_static_desc(enum_desc_t ed)
{
    enum_desc_print(stdout, ed, false) ;

    printf("val(VV3)=%d\n", enum_desc_find_by_label(ed, "VV3")) ;
    printf("str(VV3)=%d\n", enum_desc_find_by_value(ed, VV3)) ;
    printf("value(-1)=%d\n", enum_desc_find_by_value(ed, -1)) ;
    printf("str(ZZZ)=%d\n", enum_desc_find_by_label(ed, "ZZZ")) ;
    printf("str(VV2)=%s\n", enum_desc_label_of(ed, VV2)) ;
    printf("int(VV4)=%d\n", enum_desc_value_of(ed, "VV4", -9999));
}

#include "enum_desc_def.h"

static const struct enum_desc s2_desc = {
    .item_count = 4,
    .values = (const int[5]) { VV1, VV2, VV3, VV4, 0},
    .strs = "s2\0VV1\0VV2\0VV3\0VV4\0\0\0\0\0\0\0\0",
    .lbl_off = (const uint16_t[5]) { 4, 8, 12, 16, 20 },
} ;

#include "enum_refl.h"

static void test_dynamic_refl(void)
{
    enum_desc_t e1_desc = enum_refl_build("e1", (struct enum_desc_entry []) { { E1, "E1", NULL}, { E3, "E3",  NULL }, { E100, "E100", NULL}, {} }, 0, NULL) ;

    show_desc(e1_desc) ;
    {
        enum_desc_t ed = e1_desc ;
        printf("str(E3)=%d\n", enum_desc_find_by_label(ed, "E3")) ;
        printf("val(E100)=%d\n", enum_desc_find_by_value(ed, E100)) ;
        printf("val(-1)=%d\n", enum_desc_find_by_value(ed, -1)) ;
        printf("str(ZZZ)=%d\n", enum_desc_find_by_label(ed, "ZZZ")) ;
        printf("str(VV2)=%s\n", enum_desc_label_of(ed, VV2)) ;
        printf("int(VV4)=%d\n", enum_desc_value_of(ed, "VV4", -1));
    }
    enum_desc_destroy(e1_desc) ;
}


int main(int argc, char **argv)
{
    (void) argc ;
    (void) argv ;
    test_static_desc(&s2_desc) ;
    test_dynamic_refl() ;
}